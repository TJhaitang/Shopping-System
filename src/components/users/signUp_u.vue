...
export default {
  data() {
    ...
    return {
      ...
      Rules: {
        ...
        identity: [{ required: true, trigger: "blur", validator: checkIdNum }],
        name: [
  ...
  methods: {
    ...
    getCode() {
        this.$refs.signupFormRef.validateField('name',async (valid) => {
        if (!valid) return; //如果预验证失败就返回
        else this.$refs.signupFormRef.validateField('email',async (valid) => {
        if (!valid) return; 
        const result = await this.$http.post(
          "/login/getMail.php",
          this.signupForm
