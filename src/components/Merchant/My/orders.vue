...
<template>
  <div class="tab-container">
    ...
    <div class="filter-container"> 
      <el-input v-model="orderQuery.id" placeholder="订单id" style="width: 200px;margin-right:15px;margin-left:15px;margin-bottom:15px;"   />
    ...
    <el-table-column type="expand">
      <template slot-scope="props">
        <el-form  v-for="item in props.row.items" :key="item.comName"  label-position="left" inline class="demo-table-expand" style="display:flex">
          <el-form-item label="商品-规格名称" style="margin-left: 20px; margin-bottom: 0;width: 35%;">
            <span>{{ item.comName }}</span>
          </el-form-item>
          <el-form-item label="数量" style="margin-left: 20px; margin-bottom: 0;width: 35%;">
            <span>{{ item.quantity }}</span>
          </el-form-item>
        </el-form>
        <el-form  label-position="left" inline class="demo-table-expand" style="display:flex">
          <el-form-item label="总数量" style="margin-left: 20px; margin-bottom: 0;width: 35%;">
            <span>{{ props.row.itemNum }}</span>
          </el-form-item>
          <el-form-item label="总金额" style="margin-left: 20px; margin-bottom: 0;width: 35%;">
            <span>{{ props.row.cost }}</span>
          </el-form-item>
          <el-form-item label="操作" style="margin-left: 20px; margin-bottom: 0;width: 35%;">
            <span>
              <el-popover >
              <p>确认将这一单改成下一个状态吗？</p>
              ...
              <i class="el-icon-s-check" slot="reference" style="font-size: 18px;"></i>
            </el-popover></span>
          </el-form-item>
        </el-form>
      </template>
...
<script>
  ...
  methods: {
    ...
    //   }  
     ...
     for(let i = 1;i<=result.orderNum;i++){
       ...
       console.log(this.orderList[i-1]);
        if(this.orderList[i-1].status == "1") {  this.orderList[i-1].status = '待审核' }
        else if(this.orderList[i-1].status == "2"){this.orderList[i-1].status = '待发货'}
        else if(this.orderList[i-1].status == "3"){this.orderList[i-1].status = '待收货'}
        else {this.orderList[i-1].status = '订单完成'}
    ...
    //修改订单信息并提交
    toNextStatus (code) {
      this.$http.post('/merchant/changeOrderToNextStatus.php',
      {
        id:code
